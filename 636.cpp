...
public:
    vector<int> exclusiveTime(int n, vector<string>& logs) {
        ...
        vector<int> begin;
        begin.push_back(0);
        int index = 0, lastTime = 0, lastIndex = 0,lastState = 1;
        for(auto log:logs) {
            //看清题，用例范围肯定不是这么小，要进行转换，不要着急做题
            index = translateIndex(log);
            int s = log.find(":");
            if(log[s + 1] == 's') {
                begin.push_back(index);
                int t = translate(log);
                if(lastState) {
                    time[lastIndex] += t - lastTime;
                } else {
                    time[lastIndex] += t - lastTime - 1;
                }
                lastTime = t;
                lastState = 1;
                ...
                begin.pop_back();
                int t = translate(log);
                if(index == lastIndex && lastState) {
                    time[index] += t - lastTime + 1;
                } else {
                    time[index] += t - lastTime;
                }
                lastTime = t;
                lastState = 0;
            }
            lastIndex = begin.back();
        }
        return time;
    }
    ...
    //记清楚如何从字符串转换为整数
    static int translateIndex(string s) {
        int i = 0;
        int result = 0;
        while(s[i] >= '0' && s[i] <= '9') {
